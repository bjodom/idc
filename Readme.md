...
5. **Publish results:** We welcome you publishing results you get from using the system. Of course, if you see anything unexpected we would welcome [question via request support - see instructions.](#where-to-get-support). We like seeing credit for the use of Intel Developer Cloud in papers, and we encourage you to use the exclusive queue to get performance numbers without others on the system. The systems with PCIe cards are 1100 (single tile) GPUs, and we will eventually also have 1550 (dual tile) GPUs. Noting which you used is encouraged too.
